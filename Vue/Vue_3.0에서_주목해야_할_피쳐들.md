...
- Fragments & Portals: 작아진 크기에도 불구하고 3.0에서는 Fragments(컴포넌트가 여러 루트 노드를 반환함)와 Portals(컴포넌트 내부 대신 DOM의 다른 영역의 서브트리를 렌더링함)을 빌트인으로 지원합니다.
- 슬롯 메커니즘이 개선됩니다. 모든 컴파일러에 의해 생성된 슬롯은 이제 함수로 취급되고 자식 컴포넌트가 렌더 함수가 호출되는 동안 실행됩니다. 이렇게 하면 슬롯의 종속성이 부모컴포넌트 대신 자식 컴포넌트로 잡히게 됩니다. 이것은 두가지 의미를 갖습니다. 1. 슬롯의 내용이 변경되었을 때 오로지 자식 컴포넌트만 새로 렌더링 됩니다. 2. 부모 컴포넌트가 다시 렌더링 됐을 때, 슬롯에 변경사항이 없다면 자식 컴포넌트는 다시 렌더링되지 않습니다. 이 변경 사항은 컴포넌트 트리 수준에서 훨씬 더 정확한 변경 탐지를 제공하므로 쓸모없이 다시 렌더링되는 횟수를 줄일것입니다.
- 사용자 렌더러를 생성할 수 있는 1급 API가 제공됩니다. 이로 인해 더이상 vue의 코드를 포크해서 사용자가 재정의할 필요가 없어집니다. 이 변경사항은 네이티브를 위한 렌더러가 필요한 [Weex](https://weex.apache.org/)나 [NativeScript Vue](https://nativescript-vue.org/)와 같은 프로젝트가 Vue의 변경사항을 따라가는데 더 적은 노력을 들게 만들것입니다. 또한 다양한 다른 목적으로 사용자 정의 렌더러를 만드는것을 쉽게 만들것입니다.

### Compiler Improvements
tree-shaking을 보다 더 잘 지원하고, 좀 더 AOT 최적화되고, 파서는 에러정보와 소스맵을 더 잘 지원합니다.

- tree-shaking이 가능한 번들러를 사용할 때 선택적 기능을 사용하는 템플릿은 ES 모듈 구문을 통해 해당 기능을 가져오는 코드를 생성하게 됩니다. 이로인해 사용하지 않는 선택적 기능은 최종 번들파일에서 제거되게 됩니다.
- 새로운 버츄얼 돔 구현으로 인해 정적 트리 호이스팅, 정적 프로퍼티 호이스팅, 런타임을 위한 컴파일러 힌트, 자식 노드 정규화 스킵, 빠른 가상 노드 패스 생성등 좀 더 효과적인 컴파일 타임 최적화가 이루어집니다.
- 템플릿을 컴파일하다 에러가 발생했을 때 위치정보를 제공하기 위해서 파서를 다시 구현할 예정입니다. 새로 작성되는 파서는 템플릿 소스맵을 지원해야 하며 IDE나 `eslint-plugin-vue` 서드파티 도구들에게 기반을 제공해야합니다.

### IE11 Support
IE11은 여전히 지원될것입니다. 하지만 2.x 에서의 반응형 한계들이 여전히 유지될 것입니다.

새로운 코드베이스는 ES2015를 지원하는 ever green 브라우저들에 초점이 맞추어져 있습니다. 그러나 우리는 뷰 사용자들이 아직 IE11에 대한 지원이 필요하다는 사실 또한 잘 알고있습니다. 대부분의 ES2015 기능들은 IE11을 위해 트랜스파일되거나 폴리필 될 수 있지만, `Proxy`는 불가능합니다. 우리의 계획은 같은 API를 제공하는 대체 옵저버를 `proxy` 대신 `Object.defineProperty`를 사용하여 구현하여 제공할 예정입니다. 이 옵저버 구현을 사용한 Vue 3.x의 별도의 빌드가 배포됩니다. 그러나 이 빌드는 Vue 2.x와 동일한 변경 감지 한계를 갖게 되므로 3.x의 모던한 빌드와 완전히 호환되지는 않습니다. 이것이 라이브러리 제공자들이 두가지 다른 빌드에 대한 호환성을 알아야 하기 때문에 불편함을 갖는다는 사실을 인지하고 있지만, 최대한 상세히 가이드함으로써 해결할 예정입니다.
