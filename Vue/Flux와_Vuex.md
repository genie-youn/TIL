...
- 컨테이는 프로퍼티에 접근할 수 없다. - 기본적으로 컨테이너는 어떠한 프로퍼티에도 접근할 수 없다. 이는 두가지 이유가 있다. 하나는 성능적인 이유이고, 두번째는 컨테이너의 재사용을 보장하고 컴포넌트 트리 전체에 걸쳐 프로퍼티를 끼워넣을 필요가 없도록 하기 위한 것이다. 몇가지 유효한 상황이 있다. 스토어의 상태와 프로퍼티 모두를 기준으로 상태를 결정해야 하는 몇 가지 유효한 상황이 있다. 이러한 경우 `create()` 의 두번째 인자로 `{withProps: true}` 옵션을 전달하면 된다. 이렇게 하면 `calculateState()`의 두번째 인자로 컴포넌트의 프로퍼티가 노출되게 된다
